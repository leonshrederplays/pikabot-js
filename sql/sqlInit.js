module.exports = {
    sqlInit: function sqlInit(connection) {
            ...
            connection.query(sql, (err, result, fields) => {
                if (!err) {
                    if (result.warningCount < 1) {
                        ...
                        connection.query(
                            'CREATE UNIQUE INDEX guildID ON guilds (guildID)',
                            (err, result, fields) => {
                                if (!err) {
                                    ...
                                    console.log(
                                        'Something went wrong retrying in 3secs...'
                                    );
                        ...
                        console.log('Table Guilds already exists...');
                        return resolve('SUCCESS');
                    }
                } else {
                    console.log(err.sqlMessage);
                    console.log('Something went wrong retrying in 3secs...');
                    return reject('FAILED');
                }
            });
        });
    },
    setupData: function setupData(client, connection) {
        return new Promise((resolve, reject) => {
            let guildSize = client.guilds.cache.size;
            // If database is empty insert all Guilds.
            connection.query('SELECT * FROM `guilds`', (err, rows, fields) => {
                if (!err) {
                    if (rows.length === 0) {
                        if (guildSize > 0) {
                            console.log(
                                'Database is empty injecting all Guilds im currently part of...'
                            );
                        ...
                        let removedGuilds = 0;
                        console.log(
                            'Checking for Guilds i am no longer part of...'
                        );
                        ...
                        let addedGuilds = 0;
                        console.log(
                            'Checking for new Guilds added while i was offline...'
                        );
                        client.guilds.cache.forEach((guild) => {
                            let guildID = guild.id;
                            let guildName = guild.name;
                    ...
                    console.log(err.sqlMessage);
                    console.log(
                        'Something went wrong getting data retrying in 3secs...'
                    );
                    return reject('FAILED');
                }
            });
    ...
    validateData: function validateData(client, connection) {
        return new Promise((resolve, reject) => {
            ...
            client.guilds.cache.forEach((guild) => {
                ...
                if (guild.name != guild.guildSettings.guildName) {
                    connection.query(
                        'UPDATE `guilds` SET `guildName` = ? WHERE `guildID` = ?',
                        [guild.name, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Updated Guild-Name of: ${guild.name} from: ${guild.guildSettings.guildName} to: ${guild.name}`
                                );
                ...
                if (guild.ownerID != guild.guildSettings.ownerID) {
                    connection.query(
                        'UPDATE `guilds` SET `ownerID` = ? WHERE `guildID` = ?',
                        [guild.ownerID, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Updated Guild-Owner from: ${guild.name} to: ${guild.ownerID}`
                                );
                ...
                if (
                    ...
                    connection.query(
                        'UPDATE `guilds` SET `cmdChannel` = ? WHERE `guildID` = ?',
                        [null, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Reseted Command-Channel from ${guild.name}`
                                );
                                guild.guildSettings.cmdChannel = null;
                            } else {
                                console.log(err.sqlMessage);
                ...
                if (
                    ...
                    connection.query(
                        'UPDATE `guilds` SET `cmdMusicChannel` = ? WHERE `guildID` = ?',
                        [null, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Reseted MusicCommand-Channel from ${guild.name}`
                                );
                                guild.guildSettings.cmdMusicChannel = null;
                            } else {
                                console.log(err.sqlMessage);
                ...
                if (
                    ...
                    connection.query(
                        'UPDATE `guilds` SET `greetingChannelID` = ? WHERE `guildID` = ?',
                        [null, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Reseted Greeting-Channel from ${guild.name}`
                                );
                                guild.guildSettings.cmdChannel = null;
                            } else {
                                console.log(err.sqlMessage);
                ...
                if (
                    ...
                    connection.query(
                        'UPDATE `guilds` SET `leaveChannelID` = ? WHERE `guildID` = ?',
                        [null, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Reseted Leave-Channel from ${guild.name}`
                                );
                                guild.guildSettings.leaveChannelID = null;
                            } else {
                                console.log(err.sqlMessage);
                ...
                if (
                    ...
                    connection.query(
                        'UPDATE `guilds` SET `autoRoleID` = ? WHERE `guildID` = ?',
                        [null, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Reseted Autorole from ${guild.name}`
                                );
                                guild.guildSettings.autoRoleID = null;
                            } else {
                                console.log(err.sqlMessage);
                ...
                if (
                    ...
                    connection.query(
                        'UPDATE `guilds` SET `ruleRoleID` = ? WHERE `guildID` = ?',
                        [null, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Reseted Rule-Role from ${guild.name}`
                                );
                                guild.guildSettings.ruleRoleID = null;
                            } else {
                                console.log(err.sqlMessage);
                ...
                if (
                    ...
                    connection.query(
                        'UPDATE `guilds` SET `ruleChannelID` = ? WHERE `guildID` = ?',
                        [null, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Reseted Rule-Channel from ${guild.name}`
                                );
                                guild.guildSettings.ruleChannelID = null;
                            } else {
                                console.log(err.sqlMessage);
                ...
                if (
                    ...
                    connection.query(
                        'UPDATE `guilds` SET `statCategoryID` = ? WHERE `guildID` = ?',
                        [null, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Reseted Stat-Category from ${guild.name}`
                                );
                                guild.guildSettings.statCategoryID = null;
                            } else {
                                console.log(err.sqlMessage);
                ...
                if (
                    ...
                    connection.query(
                        'UPDATE `guilds` SET `statChannelID` = ? WHERE `guildID` = ?',
                        [null, guild.id],
                        (err, result) => {
                            if (!err) {
                                console.log(
                                    `Reseted Stat-Channel from ${guild.name}`
                                );
                                guild.guildSettings.statChannelID = null;
                            } else {
                                console.log(err.sqlMessage);
